    ...
    stages {
        ...
        stage('Maven Build') {
            steps {
                container('maven-jdk-node'){
                    sh 'mvn -v'
                    sh "mvn -P dev -f ./DashboardApi/pom.xml clean package"
                    sh "ls -al ./DashboardApi/target"
                    sh "pwd"
                }
            }
        }
        ...
        stage('Build Docker Image'){
            steps {
                container('docker'){
                    sh 'docker --version'
                    sh 'docker build -t harbor.okestro.cld/develop/dashboard-api:latest ./DashboardApi'
                    sh 'docker images'
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                container('docker'){
                    ...
                    sh 'docker push harbor.okestro.cld/develop/dashboard-api:latest'
                }
            }
        }
